...
(function() {
    ...
    document.addEventListener('submit', function(e) {
        const form = e.target;
        if (form.id === 'buybox-form' || form.classList.contains('ui-pdp-buybox')) {
            e.preventDefault();
            e.stopPropagation();
    ...
    // Intercepta TODOS os cliques em botões de compra
    document.addEventListener('click', function(e) {
        const target = e.target.closest('button');
        if (target && (
            target.textContent.includes('Comprar') || 
            target.textContent.includes('comprar') ||
            target.classList.contains('ui-pdp-action--primary')
        )) {
            e.preventDefault();
            e.stopPropagation();
            e.stopImmediatePropagation();
...
// Adicionar listener para fechar com tecla ESC
